...
"""Preview and download Techiaith TTS voices."""
...
from . import TechiaithTextToSpeechSystem, TECHIAITH_VOICES_DIR
...
class InstalledTechiaithVoicesPanel(SizedPanel):
    ...
    def update_voices_list(self, set_focus=False, invalidate_synth_voices_cache=False):
        voices = list(TechiaithTextToSpeechSystem.load_piper_voices_from_nvda_config_dir())
        enable = bool(voices)
        self.buttons_panel.Enable(enable)
        self.voices_list.set_objects(voices, set_focus=set_focus)
        if "techiaith" in synthDriverHandler.getSynth().name.lower():
...
class OnlineTechiaithVoicesPanel(SizedPanel):
    def __init__(self, parent):
        super().__init__(parent, -1)
        self.__already_populated = threading.Event()
        self.languages = []
...
class TechiaithVoiceManagerDialog(SimpleDialog):

    def __init__(self):
        super().__init__(
            gui.mainFrame,
            # Translators: title of voice manager dialog
            title=_("Techiaith TTS voice manager"),
        )
        self.SetSize((500, -1))
        self.CenterOnScreen()
    ...
    def addControls(self, parent):
        ...
        panel_info = [
            (
                ...
                _("Installed"),
                InstalledTechiaithVoicesPanel(self.notebookCtrl),
            ),
            (
                # Translators: label of a tab in a tab control
                _("Download"),
                OnlineTechiaithVoicesPanel(self.notebookCtrl),
            ),
        ]

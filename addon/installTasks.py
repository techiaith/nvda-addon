...
_PIPER_SYNTH_DIR = os.path.join(_DIR, "synthDrivers", "techiaith_tts")
...
def onUninstall():
    with _temporary_import_psutil() as psutil:
        force_kill_techiaith_grpc_server(psutil)


def force_kill_techiaith_grpc_server(psutil):
    log.debug("Trying to force kill Techiaith TTS GRPC server process")
